...
def main():
	...
	while True:
		if contacts or Calling:
			Calling = True
			info = 'Hola {}'.format(friend)
			info = stream_rep.read(CHUNK)
			#print(info)
			s_req.send_json({'op':'chat', 'me':name, 'friend':friend, 'info':info.decode('utf-8','ignore')})
			s_req.recv_json()
			msg = s_rep.recv_json()
			info_recv = msg['info']
			print(info_recv.encode('utf-8', 'ignore'))
			stream_recv.write(info_recv.encode('utf-8', 'ignore'))
